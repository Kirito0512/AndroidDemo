...
public class AndroidUtils {
    ...
    public static int getDeviceNavigationBarHeight() {
        Resources rs = ApplicationUtil.getInstance().getApplication().getResources();
    ...
    public static boolean hasNavigationBar(Context context) {
        if (getDeviceNavigationBarHeight() == 0)
            return false;
    ...
    private static boolean isHuaWeiHideNav(Context context) {
        return Settings.Global.getInt(ApplicationUtil.getInstance().getApplication().getContentResolver(), "navigationbar_is_min", 0) != 0;
    }
    ...
    private static boolean isXiaoMiFullScreen(Context context) {
        return Settings.Global.getInt(ApplicationUtil.getInstance().getApplication().getContentResolver(), "force_fsg_nav_bar", 0) != 0;
    }
    ...
    private static boolean isVivoFullScreen(Context context) {
        return Settings.Secure.getInt(ApplicationUtil.getInstance().getApplication().getContentResolver(), "navigation_gesture_on", 0) != 0;
    }
    ...
    public static boolean isHasNavigationBar(Context context) {
        ...
        if (displayHeight > displayWidth) {
            if (displayHeight + getDeviceNavigationBarHeight() > realHeight) return false;
        } else {
            if (displayWidth + getDeviceNavigationBarHeight() > realWidth) return false;
        }
